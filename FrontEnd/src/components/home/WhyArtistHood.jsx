...
export default function WhyAH() { return <section className="why-section"><div className="home-section"><div className="section-heading why-heading"><p>WHY ARTISTHOOD</p><h2>Booking made memorable.</h2></div><div className="feature-grid">{features.map(({ title, text, Icon }) => <article key={title}><Icon /><div><h3>{title}</h3><p>{text}</p></div></article>)}</div><div className="stats-grid">{stats.map(({ value, label, Icon }) => <div key={label}><Icon /><strong>{value}</strong><span>{label}</span></div>)}</div></div></section>; }
